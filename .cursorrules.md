...
- Target iPhone 14 Pro Max (430×932)
...
- ⚠️ Firestore is in TEST MODE - not secure for public launch
- 📱 Always test mobile responsiveness
- 💾 Never lose user data - preserve state structure
- 🔄 Every state change must call saveToFirebase()
- 🎯 Focus Mode is the core experience - protect it
...
Phase 3 (Current):
- Application status tracking (Queued → Tailoring → Ready → Sent)
...
User Preferences:
- Has OCD/perfectionism tendencies → needs randomizer, clear criteria
- Visual learner → emojis, graphics, progress bars
- Multi-device user → sync is critical
- iPhone 14 Pro Max → optimize for this screen
...
- **Page transitions**: Fade up from bottom `translateY(20px) → 0`, 400ms ease-out
...
- **Headers**: Bold weight, clear size jumps (48px → 32px → 24px → 16px)
...
- **Always test** on iPhone 14 Pro Max viewport (430×932)
